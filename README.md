...
| **Message** | A note from one agent's loop to another repo's. Queued durably — nothing needs to be listening. |
...
the end, it doesn't jump the queue.

**Agents name themselves per repo** (`recap whoami "Name" --color "#RRGGBB"`).
The name appears on comments and messages — never in git.
...
| `Space` / `^P` | command palette (themes live here — 20, live-switchable) |
...
"commit not found" banner for an unresolvable sha — never a silent "no
changes".
