...
- **You stay saturated.** Finish an item, complete it, and immediately `recap next` for the
  following one. You never block waiting for a review or an answer; when the queue runs
...
ALWAYS a `Verify:` line with the command or steps that re-prove it. Short lines, bold the
...
sentences make the same point, keep one; use a period or semicolon where an
em-dash tempts you.

Order matters: **you commit first** (yourself; see Boundaries, and don't defer it to the
...
N while you work N+1 instead of facing a backlog of nine at once. Don't barrel
task→task→task in one burst dumping a wall of items, especially when the reviewer is
actively engaged (replying, reviewing). A steady stream keeps both sides saturated; a
flood saturates only you.
...
sweep; that's the old pre-`next` workflow. Call `recap next`; it hands you the next
...
amends queue (your turn). If the reviewer's point was a **question** with nothing to
revise, answer it: reply (below) and then `recap review resolve <id>` to hand it back.
...
To answer a specific reviewer comment in place (a clarifying question, a decision
explained, a note on what you changed), reply to it rather than leaving a loose task
comment:
...
the human can see and reorder it. For work that needs agreement across repos (new
API surface, changes to a contract others consume), escalate from a message
...
**Never drop todos onto another repo's queue; the CLI refuses it.**
...
- The review db (`$RECAP_DB` or `~/.local/share/recap/recap.db`, the XDG data dir) is
  private to the reviewer and cross-repo: never commit it, never push it, never surface
